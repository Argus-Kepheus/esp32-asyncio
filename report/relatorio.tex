...
Os catorze itens a seguir descrevem \emph{scripts de diagnóstico disponíveis} no repositório (\texttt{tests/}) -- o que cada um se propõe a verificar e como --, não uma alegação de que cada um foi executado e aprovado nesta revisão. A Tabela~\ref{tab:matriz-testes}, ao final desta seção, é a fonte da verdade sobre quais desses scripts têm um resultado de execução de fato registrado; um script existir no repositório não é, por si só, evidência de que ele passou.
...
\begin{enumerate}
  ...
  \item \textbf{LEDs indicadores:} Validou apenas a fiação dos dois LEDs indicadores de estado do sistema -- laranja de barramento ocioso (GPIO~13) e amarelo de atividade do escalonador (GPIO~2) --, alternando-os em um temporizador fixo; não exercita a lógica real de ocupação/atividade de \texttt{main.py}, que depende do barramento e do escalonador em funcionamento para ter sentido.
  ...
  \item \textbf{Reserva do console para o serial (sem hardware):} Único teste puramente lógico da sequência -- não toca nenhum \texttt{Pin}/\texttt{I2C}/\texttt{SPI} e roda em CPython comum, sem Wokwi nem placa. Reproduz a lógica de \texttt{console\_log()} para confirmar que toda mensagem chega ao console serial tanto com \texttt{tft\_display} igual a \texttt{None} quanto com um objeto ativo simulando uma TFT fisicamente ausente porém eletricamente silenciosa (Seção~\ref{sec:tft-console}).
\end{enumerate}
...
Além dos catorze testes isolados, quatro procedimentos de validação integrada -- limites dos botões de velocidade, operação simultânea dos três mostradores, caminho de falha da TFT e uma execução prolongada para observar a dessincronização dos LEDs e a latência do botão -- estão definidos como TC-08 a TC-11 em \texttt{docs/EN/technical-specification.md} (§12) e como itens 14.5 a 14.8 em \texttt{docs/PT/technical-specification.md} (§14). Nenhum dos quatro foi executado nesta revisão; são procedimentos definidos para quando \texttt{main.py} completo for de fato rodado, não resultados.

\begin{table}[htbp]
  \centering
  \caption{Matriz de execução dos catorze testes isolados desta revisão.}
  ...
  \begin{tabularx}{\textwidth}{@{}C{0.5cm} >{\raggedright\arraybackslash\hsize=1.35\hsize}X C{3.3cm} C{2.1cm} >{\raggedright\arraybackslash\hsize=0.65\hsize}X@{}}
    ...
    \midrule
    1 & \texttt{01\_red\_led\_basic.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    2 & \texttt{02\_red\_led\_blink.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    3 & \texttt{03\_red\_led\_asyncio.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    4 & \texttt{04\_push\_button\_green\_led.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    5 & \texttt{05\_oled\_basic.py} & Wokwi web, data não registrada$^{2}$ & -- & Passou, historicamente$^{2}$ \\
    6 & \texttt{06\_oled\_full\_diagnostic.py} & Wokwi web, data não registrada$^{2}$ & -- & Passou, historicamente$^{2}$ \\
    7 & \texttt{07\_extra\_leds\_basic.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    8 & \texttt{08\_blinking\_leds\_asyncio.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    9 & \texttt{09\_speed\_buttons.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    10 & \texttt{10\_idle\_leds\_basic.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    11 & \texttt{11\_oled2\_basic.py} & Nenhuma nesta revisão & -- & Não avaliado$^{1}$ \\
    ...
    14 & \texttt{14\_console\_serial\_fallback.py} & Desktop, CPython$^{3}$ & N/A$^{3}$ & Passou -- saída conferida nesta revisão \\
    \bottomrule
  \end{tabularx}

  \vspace{0.3em}
  \raggedright\footnotesize
  $^{1}$Script presente no repositório e descrito nesta seção, mas sem nenhuma execução registrada nesta revisão -- não deve ser lido como aprovado até ser de fato executado (em Wokwi ou hardware real) e esta tabela ser atualizada com o resultado.\\
  $^{2}$Execução documentada no registro de decisões de \texttt{docs/PT/technical-specification.md} (§16), anterior à realocação do SCL do primeiro OLED de GPIO~25 para GPIO~32 (Seção~\ref{sec:requisitos}). O resultado permanece válido para a lógica I\textsuperscript{2}C exercida, mas não confirma a pinagem atual sem uma nova execução.\\
  $^{3}$Roda sob CPython de mesa, não MicroPython, Wokwi ou hardware real -- o script não usa nenhum módulo específico do MicroPython (Seção~\ref{sec:tft-console}).
\end{table}
...
A arquitetura assíncrona permite a concorrência de treze fluxos independentes (Seção~\ref{sec:estrutura-geral}) sob um único escalonador cooperativo. O antirrepique compartilhado elimina a necessidade de filtro externo nesta aplicação e em suas três variantes de botão, e as otimizações aplicadas ao console de registro da TFT -- tabela de conversão de cor pré-calculada, limpeza apenas do trecho residual de cada linha -- mantêm o tempo de escrita bloqueante baixo o suficiente para não dominar o próprio gráfico de uso de CPU que ele ajuda a alimentar. Por fim, os quatorze testes isolados e progressivos (Seção~\ref{sec:testes}) forneceram evidências de funcionamento de cada subsistema antes da integração completa.
