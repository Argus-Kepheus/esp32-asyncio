...
\paragraph{Nota sobre o estado do projeto.} As Seções~\ref{sec:requisitos} a~\ref{sec:testes} deste relatório descrevem o escopo original do projeto -- dois LEDs, um botão e um OLED -- que está completo, validado e estável. A partir dele, a pedido do usuário, o hardware e o \texttt{main.py} foram estendidos experimentalmente com LEDs adicionais, um segundo OLED, um display TFT, botões extras e outros componentes; essa extensão \textbf{ainda está em desenvolvimento ativo} e não é redescrita em detalhe aqui, para evitar que este relatório fique desatualizado a cada iteração. O estado atual e completo dessa extensão é mantido em \texttt{docs/EN/technical-specification.md} (Seção~19, \emph{Extended features}) e seu equivalente em \texttt{docs/PT} (Seção~17). A Figura~\ref{fig:circuito}, na Seção~\ref{sec:circuito}, já reflete o circuito atual, incluindo essa extensão.
...
\begin{description}
  \item[Sinalização periódica do LED vermelho:] Alterna continuamente, independentemente do botão, do LED verde e do OLED, entre os estados \textbf{desligado} (GPIO~26 em nível baixo por aproximadamente 500~[ms]) e \textbf{ligado} (GPIO~26 em nível alto por aproximadamente 500~[ms]), compondo um ciclo completo de aproximadamente 1~[s].
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}Y C{1.6cm} C{2.2cm} Y Y@{}}
    ...
    \midrule
    LED vermelho & 26 & J2-10 & \texttt{red-led} & \texttt{red\_led} \\
    ...
    OLED, relógio I\textsuperscript{2}C & 32 & J2-7 & \texttt{oled-display:SCL} & \texttt{OLED\_SCL\_PIN} \\
    \bottomrule
  \end{tabularx}
...
O LED vermelho e o SCL do OLED foram realocados dos GPIO~2 e GPIO~25, originalmente fixados antes do início do desenvolvimento, para os GPIO~26 e GPIO~32 acima, a pedido explícito do usuário, por motivos de layout da placa à medida que mais periféricos foram adicionados (ver a nota da Seção~\ref{sec:requisitos} e o registro de decisões em \texttt{docs/EN/technical-specification.md}, §16). A Tabela~\ref{tab:pinagem} reflete a fiação atual, não a atribuição original.
...
\begin{enumerate}
  \item \textbf{Teste isolado do LED vermelho:} Utilizou um laço bloqueante mínimo somente para diagnóstico. O GPIO~26 foi alternado a cada 500~[ms] e o estado foi registrado no monitor serial. O objetivo foi confirmar placa, pino, resistor, polaridade e retorno ao terra sem interferência das demais funções.
...
Os seis testes acima cobrem o escopo original do projeto. Sete testes adicionais (\texttt{tests/07} a \texttt{tests/13}) foram criados posteriormente, seguindo a mesma lógica progressiva, para os LEDs extras, os botões de velocidade, os LEDs indicadores, o segundo OLED e a TFT da extensão experimental mencionada na Seção~\ref{sec:requisitos}; não são detalhados aqui pelo mesmo motivo -- ver \texttt{tests/README.md} para a lista completa e as dependências entre eles.

\section{Circuito simulado}
\label{sec:circuito}

A Figura~\ref{fig:circuito} apresenta o estado atual do circuito no Wokwi, já incluindo a extensão experimental mencionada na nota da Seção~\ref{sec:requisitos}. Com o número maior de componentes e sinais, as cores dos condutores deixaram de seguir uma convenção única por função; o mapeamento completo de pinos e sinais é mantido em \texttt{docs/EN/technical-specification.md} (Seção~19) e \texttt{docs/PT/technical-specification.md} (Seção~17), não reproduzido aqui.
...
\begin{figure}[htbp]
  ...
  \caption{Circuito do projeto no Wokwi: ESP32-DevKitC V4, dois OLEDs SSD1306, display TFT ILI9341, seis LEDs azuis, um LED verde, três botões pulsadores e uma chave deslizante.}
  \label{fig:circuito}
\end{figure}
